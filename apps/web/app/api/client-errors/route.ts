...
import { safeJson } from '../../../lib/live/log';
 ...
 * logged, 400 when there's nothing to log (a body cut off part way included),
 * 413 when the body is over 4 KB. Rate limiting belongs at the edge, like
 * every other open route here.
 */
export async function POST(req: Request): Promise<Response> {
  ...
  console.error(safeJson({ event: 'client_error', ...report, userAgent: clip(req.headers.get('user-agent')) }));
  return new Response(null, { status: 204 });
}

/**
 * The body as text, or null once it passes `max` bytes: a large Content-Length
 * is refused unread, and a body without one is read no further than the
 * limit. A body cut off part way, by a phone that left mid-report, reads as
 * empty: there's nothing whole to log, and a line about it would read as a
 * fault of ours.
 */
